...
{
    private const TEMPLATE_VERSION = '2026-03-23-transcript-pdf-v1';

    public function render(GraduationDocument $document): string
    {
        if ($document->document_type !== GraduationDocumentType::TRANSCRIPT) {
            throw new \InvalidArgumentException('TranscriptPdfService hanya menerima document_type transcript.');
        }
        ...
        $data = [
            ...
            'summary' => $summary,
            'qrCode' => $qrCode,
            'letterheadPath' => $this->optimizePdfAsset($this->resolveStorageAssetPath($school['kop_surat'] ?? null), 'transcript-letterhead', 1800, 420),
            ...
            'ttdKepsekPath' => $this->optimizePdfAsset($this->resolveStorageAssetPath($school['ttd_kepsek'] ?? null), 'transcript-ttd-kepsek', 520, 180),
            'stempelPath' => $this->optimizePdfAsset($this->resolveStorageAssetPath($school['stempel_sekolah'] ?? null), 'transcript-stempel', 360, 360),
        ];
    ...
    private function buildCacheRelativePath(GraduationDocument $document, array $payload): string
    {
        ...
        return 'graduation-documents/transcript/document-' . $document->id . '/' . $fingerprint . '.pdf';
    }
